...
## 18. Publication boundary""",
    "skill inverse reconciliation",
)
s = replace_once(s, "## 18. Memory gates", "## 19. Memory gates", "skill gate numbering")
s = replace_once(
    s,
...
p = Path("_scripts/_internal/validate_structure.py")
s = p.read_text(encoding="utf-8")
s = replace_once(
    s,
...
''',
    '''            status = fields.get("status")
            ...
            for required_scalar in ("category", "summary"):
''',
    "validator semantic refs",
)
p.write_text(s, encoding="utf-8")

# Deterministic impacted-finding discovery
Path("_scripts/_internal/memory_reconciliation.py").write_text('''#!/usr/bin/env python3
...
Path("_scripts/test_memory_reconciliation.py").write_text('''#!/usr/bin/env python3
from __future__ import annotations
